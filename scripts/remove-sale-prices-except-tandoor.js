/**
 * Remove Sale Prices from All Products EXCEPT 'Electric Tandoor'
 * Uses environment variables from .env.local or Vercel
 * Run: node scripts/remove-sale-prices-except-tandoor.js
 */
...
if (!CONSUMER_KEY || !CONSUMER_SECRET) {
    ...
    console.error('Or run: WORDPRESS_CONSUMER_KEY=ck_xxx WORDPRESS_CONSUMER_SECRET=cs_xxx node scripts/remove-sale-prices-except-tandoor.js\n');
    process.exit(1);
}
    ...
    try {
        ...
        const productsWithSales = allProducts.filter(
            (product) => {
                ...
                const isTandoor = product.name.toLowerCase().includes('electric tandoor');
                return hasSale && !isTandoor;
            }
        );

        console.log(`📊 Products with sale prices (excluding Tandoor): ${productsWithSales.length}\n`);
...
async function removeSalePricesFromVariations() {
    try {
        ...
        for (const product of allVariableProducts) {
            if (product.name.toLowerCase().includes('electric tandoor')) {
                console.log(`   SKIP: ${product.name} (Electric Tandoor preserved)`);
                continue;
            }
            ...
            try {
                ...
                if (variationsWithSales.length > 0) {
                    ...
                    for (const variation of variationsWithSales) {
                        await wcApi(`products/${product.id}/variations/${variation.id}`, 'PUT', {
                            sale_price: '',
                            date_on_sale_from: null,
                            date_on_sale_to: null
                        });
...
async function main() {
    console.log('\n🚀 WooCommerce Sale Price Remover (Except Electric Tandoor)\n');
    console.log('═'.repeat(80));
